...
mod tests {
    ...
    /// Tests for CUDA backend configuration and compile-time correctness.
    /// Actual GPU tests only run when CUDA hardware is available.
    ...
    fn test_solver_stats_defaults() {
        ...
        assert_eq!(stats.num_symbolic, 0);
        assert_eq!(stats.num_numeric, 0);
        assert_eq!(stats.num_solves, 0);
        assert_eq!(stats.time_symbolic, 0.0);
    }

    #[test]
    fn test_factorization_struct() {
        let fact = CudaFactorization {
            n: 10,
            nnz_l: 50,
            success: true,
            num_neg_pivots: 2,
